...
export const GetAvailableRooms = async ({
  ...
  try {
    ...
    // Case-insensitive search on name if provided
    if (name) {
      query.name = { $regex: new RegExp(name, "i") }; // "i" flag for case-insensitive
    }

    // If check-in and check-out dates are missing, filter by name and type only
    if (!reqCheckInDate || !reqCheckOutDate) {
      const rooms = await RoomModel.find(query).populate("hotel");
      return {
        success: true,
        data: JSON.parse(JSON.stringify(rooms)),
      };
    ...
    const bookedSlots = await BookingModel.find({
      bookingStatus: "Booked",
      $or: [
        { checkInDate: { $gte: reqCheckInDate, $lte: reqCheckOutDate } },
        { checkOutDate: { $gte: reqCheckInDate, $lte: reqCheckOutDate } },
        {
          $and: [
            { checkInDate: { $lte: reqCheckInDate } },
            { checkOutDate: { $gte: reqCheckOutDate } },
    ...
    // Exclude booked rooms from the result
    const rooms = await RoomModel.find({
      _id: { $nin: bookedRoomIds },
      ...query,
    }).populate("hotel");
